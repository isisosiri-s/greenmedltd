...
// Consent checkbox icons (Tabler: "square" unchecked / "square-check" filled checked).
const ICON_SQUARE = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M3 5a2 2 0 0 1 2 -2h14a2 2 0 0 1 2 2v14a2 2 0 0 1 -2 2h-14a2 2 0 0 1 -2 -2v-14" /></svg>`;
const ICON_SQUARE_CHECK = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M18.333 2c1.96 0 3.56 1.537 3.662 3.472l.005 .195v12.666c0 1.96 -1.537 3.56 -3.472 3.662l-.195 .005h-12.666a3.667 3.667 0 0 1 -3.662 -3.472l-.005 -.195v-12.666c0 -1.96 1.537 -3.56 3.472 -3.662l.195 -.005h12.666zm-2.626 7.293a1 1 0 0 0 -1.414 0l-3.293 3.292l-1.293 -1.292l-.094 -.083a1 1 0 0 0 -1.32 1.497l2 2l.094 .083a1 1 0 0 0 1.32 -.083l4 -4l.083 -.094a1 1 0 0 0 -.083 -1.32z" /></svg>`;
const CONSENT_ICONS = `<span class="check-icon check-icon-off" aria-hidden="true">${ICON_SQUARE}</span><span class="check-icon check-icon-on" aria-hidden="true">${ICON_SQUARE_CHECK}</span>`;
...
function headExtras({ canonicalUrl, altUrl, lang, title, desc }) {
  ...
  <script type="text/javascript">
      ...
      })(window, document, "clarity", "script", "ximldvfgop");
  </script>
  <!-- /Microsoft Clarity -->
  <meta charset="UTF-8">
...
const PAGE_STYLE = `  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .consent-check .check-icon {
      position: absolute; left: 0; top: 1px;
      width: 18px; height: 18px;
      color: var(--c-muted);
    }
    .consent-check .check-icon svg { display: block; width: 100%; height: 100%; }
    .consent-check .check-icon-on { display: none; color: var(--gm-400); }
    .consent-check input:checked + label .check-icon-off { display: none; }
    .consent-check input:checked + label .check-icon-on { display: block; }
    .consent-check input:focus-visible + label .check-icon-off,
    .consent-check input:focus-visible + label .check-icon-on {
      outline: 2px solid var(--gm-400); outline-offset: 2px; border-radius: 3px;
    }
